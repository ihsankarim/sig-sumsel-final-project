...
<body>
    ...
    <!-- Title Section -->
    <div class="title">
        <h1>Sumatera Selatan Berdasarkan Restoran</h1>
    </div>    <div id="map"></div>
    ...
    const sumselData = sumsels.map(sumsel => ({
        ...
        properties: {
            name: sumsel.name,
            id: sumsel.id,
            jumlah_restoran: sumsel.jumlah_restoran,
        },
        geometry: {
    ...
    function style(feature) {
        return {
            fillColor: getColor(feature.properties.jumlah_restoran),
            weight: 2,
            opacity: 1,
            color: 'white',
            dashArray: '3',
    ...
    info.update = function (props) {
        this._div.innerHTML = '<h4>South Sumatra Province Restaurants</h4>' + 
            (props ? 
                '<b>' + props.name + '</b><br />' + 
                (props.jumlah_restoran ? props.jumlah_restoran.toLocaleString('id-ID') : 'Data tidak tersedia') + ' restoran' : 
                'Hover over a sumsel');
    };
    ...
    legend.onAdd = function (map) {
        var div = L.DomUtil.create('div', 'info legend'),
            grades = [0, 20, 50, 80, 100, 150, 200, 300],
            labels = [];
